...
public class ReactiveAccountDao {
    ...
    public Mono<Account> findById(long id) {
        ...
        return Mono.from(connectionFactory.create())
          .flatMap(c -> Mono.from(c.createStatement("select id,iban,balance from jorge where id = $1")
            .bind("$1", id)
            .execute())
    ...
    public Flux<Account> findAll() {
        ...
        return Mono.from(connectionFactory.create())
            .flatMap((c) -> Mono.from(c.createStatement("select id,iban,balance from jorge")
                .execute())
                .doFinally((st) -> close(c)))
